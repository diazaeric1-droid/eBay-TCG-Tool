...
def mock_process_card(image_file):
    ...
    # Generate 90 days (3 months) of mock timeline data (roughly 12 data points)
    today = datetime.now()
    dates = [(today - timedelta(days=90 - i*8)).strftime("%b %d") for i in range(12)]
    sold_prices = [65, 70, 75, 68, 80, 85, 90, 85, 100, 110, 105, 115]
    
    # Generate corresponding sold listings table data (Reverse order: newest first)
    sold_listings = []
...
col1, col2 = st.columns([1, 1.2]) # Made the right column slightly wider to accommodate tables
...
with col2:
    ...
    if 'result' in st.session_state:
        ...
        st.info("Upload an image and click 'Process Card' to see the updated professional listing and market tables.")